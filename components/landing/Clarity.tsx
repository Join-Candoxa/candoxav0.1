  ...
  return (
    <section className="bg-black py-28 px-6">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        ...
        >
          {/* Card header */}
          <div className="px-8 py-6 border-b border-white/5 text-center">
          ...
          <div className="px-8 py-4">
            {receiptRows.map((row, i) => (
              ...
              >
                <span className={`text-sm ${row.highlight ? 'text-blue-400' : 'text-blue-400/70'}`}>
                  {row.label}
                </span>
                <span className={`text-sm font-medium ${row.highlight ? 'text-white' : 'text-white/80'}`}>
                  {row.value}
                </span>
              </div>
            ))}
