...
export default function DashboardHome({ user }: { user: any }) {
  ...
  return (
    <div className="flex gap-5">
          ...
          <div className="w-11 h-11 rounded-full flex-shrink-0 overflow-hidden bg-white/10">
            {profile?.avatar_url || user?.user_metadata?.avatar_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-blue-700">
                <span className="text-white text-[15px] font-bold">{displayName[0]?.toUpperCase()}</span>
              </div>
            )}
          </div>
          <div>
            ...
            <p className="text-white/40 text-[12px] mt-0.5">Your permanent identity is ready.</p>
          </div>
        </div>

        {/* Mobile Profile Strength card */}
        <div className="md:hidden mb-5 border border-white/[0.10] bg-[#0A0A0F]" style={{ borderRadius:'12px', padding:'14px 16px' }}>
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-white/55 text-[12px] font-medium">Profile Strength</span>
            <span className="text-white text-[12px] font-semibold">
              {profile?.profile_strength || 0}{' '}
              <span style={{ color:'#6B8AFF' }}>{strengthTier(profile?.profile_strength || 0)}</span>
            </span>
          </div>
          ...
          <>
            {entries.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                ...
                <h3 className="text-white text-[17px] font-bold mb-2">No secured entries yet</h3>
                ...
                <button onClick={() => setShowAddEntry(true)}
                  style={{ background:'#0038FF', borderRadius:'300px', padding:'14px 32px' }}
              ...
              <div className="space-y-4">
                {entries.map((entry) => (
                  <div key={entry.id} className="border border-white/[0.10] bg-[#0A0A0F]"
      ...
      {showAddEntry && (
        <AddEntryModal user={user} profile={profile}
          onClose={() => setShowAddEntry(false)}
          onSuccess={() => { setShowAddEntry(false); refetchEntries() }}
        />
      )}
    </div>
  )
