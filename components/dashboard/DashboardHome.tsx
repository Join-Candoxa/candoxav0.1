...
export default function DashboardHome({ user }: { user: any }) {
  ...
  return (
    <div className="flex gap-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-white/40 text-[14px] mt-1">Here's what's happening with your identity today.</p>
          </div>

          {/* Add Entry — #0038FF, 167×52, border-radius 300px, padding 18px 32px */}
          <button
        ...
        {/* Tabs — text #C9D1FF, active underline #0038FF, base line #FFFFFF */}
        <div className="relative mb-6">
          ...
          {/* Base border line — white, doesn't run to edge */}
          <div className="absolute bottom-0 left-0 h-[1px]" style={{ width: '480px', background: '#FFFFFF', opacity: 0.12 }} />
        </div>
        ...
        {activeTab === 'secured' && (
          <>
            {/* Entries */}
            {entries.length === 0 ? (
          /* Empty state card — uses same card spec */
          <div
            className="border border-white/[0.10] flex flex-col items-center justify-center"
            style={{ borderRadius: '12px', padding: '20px', minHeight: '200px' }}
          >
            <p className="text-white/30 text-[13px] mb-4">No secured entries yet.</p>
            <button
              onClick={() => setShowAddEntry(true)}
              style={{ background: '#0038FF', borderRadius: '300px', padding: '12px 28px' }}
              className="text-white text-[13px] font-semibold hover:opacity-90 transition-opacity"
            >
          ...
          <div className="space-y-5">
            {entries.map((entry) => (
              /* Card: 760×414, border-radius 12px, border-width 1px, padding 20px, gap 20px */
              <div
                key={entry.id}
                className="border border-white/[0.10] bg-[#0A0A0F]"
                style={{ borderRadius: '12px', padding: '20px', gap: '20px', display: 'flex', flexDirection: 'column' }}
              >
                ...
                {/* Screenshot image — 720×264, border-radius 12px */}
                {entry.screenshot_url && (
                  <img
                    src={entry.screenshot_url}
                    alt="screenshot"
                    style={{ width: '100%', height: '264px', borderRadius: '12px', objectFit: 'cover', flexShrink: 0 }}
                  />
                )}
      ...
      {/* ── Right sidebar — only shown on All Secured tab ── */}
      {activeTab === 'secured' && (
      <div className="flex-shrink-0 flex flex-col gap-4" style={{ width: '320px' }}>

        {/* Profile Strength — 320×223, border-radius 16px, border 1px, padding 20px, gap 16px */}
        <div
        ...
        {/* Trending Builders — 320×284, border-radius 16px, border 1px, padding 15px, gap 16px */}
        <div
          className="border border-white/[0.10] bg-[#0A0A0F] flex flex-col"
        ...
        {/* Platform Update — 320×133, border-radius 12px, border 1px, padding 10px, gap 7px */}
        <div
          className="border border-white/[0.10] bg-[#0A0A0F] flex flex-col"
