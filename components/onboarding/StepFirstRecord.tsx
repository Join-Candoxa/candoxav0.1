  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const { error: insertErr } = await supabase.from('entries').insert({
        user_id:        profile.id,
        title:          url, // use URL as title until metadata fetch
  ...
  return (
    <div className="w-full max-w-lg bg-[#0D0D1A] border border-white/10 rounded-2xl p-8 max-h-[90vh] overflow-y-auto">
      ...
      <div
        ...
        className="w-full border border-dashed border-white/10 rounded-xl py-10 flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 transition-colors mb-8 overflow-hidden"
      >
        {preview
