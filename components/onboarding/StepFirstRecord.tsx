...
export default function StepFirstRecord({ data, setData, onNext, onBack }: Props) {
  ...
  const ensureUserRow = async (session: any) => {
    const { data: existing } = await supabase.from('users').select('id').eq('email', session.user.email).single()
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // Return entry data so parent can show success screen
  ...
  return (
    <div className="w-full max-w-sm flex flex-col">
      ...
      <p className="text-white/40 text-[14px] mb-7">
        Attach a real achievement to your permanent identity
      </p>
      ...
      <div
        onClick={() => document.getElementById('ob-screenshot')?.click()}
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          {loading
            ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin inline-block" /> Securing...</span>
            : 'Continue →'
          }
        </button>
