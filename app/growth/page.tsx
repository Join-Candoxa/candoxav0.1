...
// ─── Circular progress (Daily Credits) ────────────────────────────────────────
...
function ValidationModal({ slot, onClose }: { slot: InviteSlot; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-[#0e0e14] border border-white/[0.12] rounded-2xl w-[400px] p-6 relative">
        ...
        <h2 className="text-white font-bold text-[17px] mb-5">Invite Successfully Validated</h2>
        ...
        <div className="border border-white/[0.08] rounded-xl overflow-hidden mb-4">
          {days.map((d, i) => (
            <div key={d.label} className={`flex items-center justify-between px-4 py-3 ${i < days.length - 1 ? 'border-b border-white/[0.06]' : ''}`}>
              ...
              {d.done
                ? <span className="text-green-400 text-[12px] font-semibold flex items-center gap-1.5"><span>✓</span> Completed</span>
...
function InviteSlotCard({ slot, onReveal, onClick }: {
  slot: InviteSlot
  onReveal: (slot: number) => void
  onClick: (slot: InviteSlot) => void
}) {
  ...
  return (
    <div
      className={`border rounded-2xl p-4 flex flex-col gap-3 ${slot.status === 'pending' ? 'border-orange-500/20 cursor-pointer hover:border-orange-500/40 transition-colors' : slot.status === 'validated' ? 'border-green-500/20' : 'border-white/[0.10]'}`}
      onClick={slot.status === 'pending' ? () => onClick(slot) : undefined}
    >
      ...
      {slot.status === 'available' && (
        <>
          <div className="flex items-center justify-between bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2.5">
            <span className="text-white/20 text-[13px] tracking-[0.25em] font-mono select-none">
              {revealed ? slot.code : '●●●●● ●●●●●●'}
            </span>
            <button
              onClick={(e) => { e.stopPropagation(); setRevealed(true); onReveal(slot.slot) }}
        ...
        <>
          <div className="bg-white/[0.04] border border-orange-500/20 rounded-xl px-3 py-2.5">
            <div className="flex items-center gap-2">
              <span className="text-orange-300 text-[13px] font-mono font-semibold">{slot.code}</span>
            </div>
          ...
          <div className="flex gap-1.5">
            {[1,2,3].map((d) => (
              <div key={d} className={`flex-1 h-1 rounded-full ${d <= slot.days_completed ? 'bg-green-400' : 'bg-white/[0.10]'}`} />
            ))}
          </div>
...
export default function GrowthPage() {
  ...
  const [activeSlot,   setActiveSlot]   = useState<InviteSlot | null>(null)
  ...
  const fetchAll = async (prof: any) => {
    ...
    // Referrals — from invite_codes table
    const { count: refCount } = await supabase
      .from('invite_codes').select('*', { count:'exact', head:true })
      .eq('user_id', prof.id).neq('status', 'available')
    setReferralCount(refCount || 0)

    // Invite slots — fetch or create 3 slots
    const { data: existingSlots } = await supabase
      .from('invite_codes').select('*').eq('user_id', prof.id).order('slot')

    if (!existingSlots || existingSlots.length < 3) {
      // Create missing slots
      const existing = existingSlots || []
      const existingNums = existing.map((s: any) => s.slot)
      const toCreate = [1,2,3].filter(n => !existingNums.includes(n)).map(n => ({
        user_id: prof.id,
        slot: n,
        code: generateCode(),
        status: 'available',
      ...
      if (toCreate.length > 0) {
        ...
        const { data: fresh } = await supabase.from('invite_codes').select('*').eq('user_id', prof.id).order('slot')
        setSlots(mapSlots(fresh || []))
      } else {
        setSlots(mapSlots(existing))
      }
    } else {
      setSlots(mapSlots(existingSlots))
    }
  }
  ...
  return (
    <DashboardLayout user={user}>
      <div className="max-w-[980px]">
        ...
        <div className="grid grid-cols-2 gap-4 mb-7">
          ...
          <div className="border border-white/[0.10] bg-[#0A0A0F] rounded-2xl p-5">
            ...
            <div className="w-full h-[3px] bg-white/[0.07] rounded-full overflow-hidden mb-3">
              <div className="h-full rounded-full transition-all duration-700" style={{ width: `${strengthPct}%`, background: '#0038FF' }} />
            </div>
          ...
          <div className="border border-white/[0.10] bg-[#0A0A0F] rounded-2xl p-5">
            ...
            <p className="text-white/30 text-[12px]">
              {totalEntries === 0 ? 'No entries yet' : totalEntries === 1 ? 'First entry this week' : `${totalEntries} total entries`}
            </p>
          </div>
          ...
          <div className="border border-white/[0.10] bg-[#0A0A0F] rounded-2xl px-5 py-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-semibold text-[14px]">
                {dailyLimit - dailyUsed} Free Secures Entries Remaining Today
              </p>
              <p className="text-white/35 text-[12px] mt-0.5">each entries anchors one record permanently to your identity.</p>
        ...
        {/* ── Invite Builders ── */}
        <div>
          <h2 className="text-white text-[18px] font-bold mb-1">Invite Builders <span className="text-white/40 font-normal">(Testnet Access)</span></h2>
          <p className="text-white/35 text-[13px] mb-5">Each user has 3 controlled invite codes. Successful invites require 1 secured entry per day for 3 consecutive days.</p>
        ...
        {/* Validation modal */}
        {activeSlot && (
          <ValidationModal slot={activeSlot} onClose={() => setActiveSlot(null)} />
        )}
      </div>
    </DashboardLayout>
  )
