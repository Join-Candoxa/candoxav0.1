...
// Track/Tracking pill button — real state, no mock
...
export default function DiscoverPage() {
  ...
  const fetchAll = async (prof: any) => {
    ...
    // Featured — top 1 by profile_strength, not self
    ...
    // New entries — latest from ALL users except self
    const { data: entries } = await supabase.from('entries')
      .select('id, title, description, platform, screenshot_url, secured_at, url, user_id, users(id, username, avatar_url)')
      .neq('user_id', prof.id).order('secured_at', { ascending: false }).limit(10)
    ...
    ;(ap||[]).forEach((e:any) => { const k=(e.platform||'other').toLowerCase(); pmap[k]=(pmap[k]||0)+1 })
    ...
    const labelMap: Record<string,string> = { twitter:'Twitter', x:'X (Twitter)', youtube:'YouTube', instagram:'Instagram', linkedin:'LinkedIn', github:'GitHub' }
  ...
  const FILTERS: { key: FilterType; label: string }[] = [
    { key: 'all', label: 'All' }, { key: 'platform', label: 'Platform' },
    { key: 'content', label: 'All Content' }, { key: 'most', label: 'Most Secured' },
        ...
        <div className="flex-1 min-w-0">
          ...
          {featured.length > 0 && (
            <div className="mb-7">
              ...
              {featured.map((c) => {
                const latest = newEntries.find(e => e.user_id === c.id)
                return (
                    ...
                    {latest && (
                      <div className="px-4 pb-4 border-t border-white/[0.06] pt-3">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => router.push(`/${c.username}`)} className="text-white/35 text-[12px] hover:text-white transition-colors">identify →</button>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          {/* NEW ENTRIES */}
          <div>
            <p className="text-white/55 text-[11px] font-bold tracking-[0.14em] uppercase mb-4">New Entries</p>
            {newEntries.length === 0
              ? <p className="text-white/20 text-[13px]">No entries from other creators yet.</p>
              : (
                  ...
                  {newEntries.map((entry) => (
                    <div key={entry.id} className="border border-white/[0.10] bg-[#0A0A0F] rounded-2xl p-4 flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        ...
                        {/* Track/Tracking based on real state — NOT hardcoded */}
                      ...
                      <div className="flex items-start gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-white/[0.06] flex items-center justify-center flex-shrink-0 overflow-hidden">
                          <Image src={platformIcon(entry.platform)} alt={entry.platform} width={18} height={18} />
                        </div>
                        <div className="min-w-0">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => router.push(`/${entry.users?.username}`)} className="text-white/35 text-[12px] hover:text-white transition-colors">identify →</button>
                      </div>
                    </div>
                  ))}
                </div>
