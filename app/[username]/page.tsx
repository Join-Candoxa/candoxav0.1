...
function ContactRequestModal({ targetProfile, myProfile, entries, onClose, onSent }: {
  targetProfile: any; myProfile: any; entries: any[]; onClose: () => void; onSent: (convoId?: string) => void
}) {
  const [contextType,    setContextType]    = useState<'entry'|'general'>('general')
  ...
  useEffect(() => {
    ...
    supabase.from('conversations').select('id')
      ...
      .then(({ data }) => { if (data) setExistingConvo(data.id) })
  }, [])
          ...
          <div className="flex items-center justify-between p-5 border-b border-white/[0.07]">
            <div>
              ...
              <p className="text-white/40 text-[12px] mt-0.5">
                {existingConvo ? 'You already have an open conversation.' : 'Send a contextual request. They can accept or decline.'}
              </p>
            </div>
            <button onClick={onClose} className="w-7 h-7 rounded-full bg-white/[0.07] flex items-center justify-center text-white/50 hover:text-white transition-colors">✕</button>
          </div>
          ...
          {existingConvo ? (
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 bg-white/[0.04] rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-white font-medium text-[14px]">@{targetProfile.username}</p>
                  <p className="text-white/35 text-[11px]">{targetProfile.bio?.slice(0, 50) || 'No bio'}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  {[
                    ...
                    { val:'entry',   label:'Select Secured Entry' },
                  ].map(({ val, label }) => (
                ...
                {contextType === 'entry' && entries.length > 0 && (
                ...
                {contextType === 'entry' && entries.length === 0 && (
                  <p className="text-white/30 text-[12px] text-center py-3">No secured entries yet.</p>
                )}
              </div>

              {/* Message */}
              <div>
                <p className="text-white/55 text-[12px] font-semibold mb-2">Message</p>
                <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3}
                  placeholder="Why are you reaching out about this record?"
              ...
              <div className="flex gap-3">
                <button onClick={onClose}
                  className="flex-1 py-3 rounded-xl border border-white/[0.12] text-white/55 text-[13px] font-semibold">
                  Cancel
                </button>
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      supabase.from('users').select('*').eq('email', session.user.email).single()
        .then(({ data }) => {
          ...
          if (data) {
            supabase.from('entries').select('id, title, platform').eq('user_id', data.id)
              .order('secured_at', { ascending:false }).limit(20)
              .then(({ data: e }) => setMyEntries(e || []))
          }
        })
    })
  }, [])
          ...
          <div className="flex items-start justify-between mb-3">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              <button onClick={copyProfileLink}
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="w-3.5 h-3.5"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                {copied === 'profile' ? 'Copied!' : 'Copy'}
              </button>
              {isOwn && (
                <button onClick={copyReferralLink}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/[0.12] text-white/55 text-[12px] font-medium">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="w-3.5 h-3.5"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="M8.59 13.51l6.83 3.98M15.41 6.51L8.59 10.49"/></svg>
              ...
              {!isOwn && (
                <>
                  ...
                  <button onClick={toggleTrack}
                    ...
                    style={isTracking ? { background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.15)' } : { background:'#0038FF' }}>
            ...
            {activity.length === 0 ? <p className="text-white/20 text-[13px] text-center py-10">No activity yet.</p>
              : activity.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  {item.points > 0 && <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-green-500/15 text-green-400 border border-green-500/20 flex-shrink-0">+{item.points}pts</span>}
                </div>
              ))
            }
          </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
                    ...
                    {!isOwn && (
                      <button onClick={() => setShowMessage(true)}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-white/[0.12] text-white/55 text-[13px] font-medium hover:border-blue-500/40 hover:text-blue-300 transition-colors">
                        <MsgIcon /> Message
                      </button>
                    )}
                    {!isOwn && (
            ...
            <div className="space-y-3">
              {activity.length===0?<p className="text-white/20 text-[13px]">No activity yet.</p>
                ...
                <div key={idx} className="flex items-center gap-4">
                  ...
                  {item.points>0&&<span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-green-500/15 text-green-400 border border-green-500/20 flex-shrink-0">+{item.points}pts</span>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex-shrink-0 flex flex-col gap-4" style={{ width:'280px' }}>
          ...
          {trackers.length > 0 && (
            <div className="border border-white/[0.10] bg-[#0A0A0F] rounded-2xl p-4 flex flex-col gap-3">
              ...
              {trackerCount > 4 && (
                <button className="text-white/35 text-[11px] hover:text-white/60 transition-colors text-center">+{trackerCount-4} more trackers →</button>
              )}
            </div>
          )}
        </div>
